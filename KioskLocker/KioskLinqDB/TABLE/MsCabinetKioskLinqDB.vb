...
Namespace TABLE
    ...
    Public Class MsCabinetKioskLinqDB
       ...
       'Define Public Method 
        ...
        '/// <param name=trans>The System.Data.SQLClient.SQLTransaction used by this System.Data.SQLClient.SQLCommand.</param>
        '/// <returns>true if data is retrieved successfully; otherwise, false.</returns>
        Public Function GetDataByPK(cID As Long, trans As SQLTransaction) As MsCabinetKioskLinqDB
            Dim p(1) As SQLParameter
            p(0) = DB.SetBigInt("@_ID", cID)
        ...
        '/// Returns an indication whether the record of MS_CABINET by specified MS_CABINET_ID key is retrieved successfully.
        '/// <param name=cMS_CABINET_ID>The MS_CABINET_ID key.</param>
        '/// <param name=trans>The System.Data.SQLClient.SQLTransaction used by this System.Data.SQLClient.SQLCommand.</param>
        '/// <returns>true if data is retrieved successfully; otherwise, false.</returns>
        Public Function ChkDataByMS_CABINET_ID(cMS_CABINET_ID As Long, trans As SQLTransaction) As Boolean
            Dim cmdPara(2)  As SQLParameter
            cmdPara(0) = DB.SetText("@_MS_CABINET_ID", cMS_CABINET_ID) 
            Return doChkData("MS_CABINET_ID = @_MS_CABINET_ID", trans, cmdPara)
        End Function

        '/// Returns an duplicate data record of MS_CABINET by specified MS_CABINET_ID key is retrieved successfully.
        '/// <param name=cMS_CABINET_ID>The MS_CABINET_ID key.</param>
        '/// <param name=trans>The System.Data.SQLClient.SQLTransaction used by this System.Data.SQLClient.SQLCommand.</param>
        '/// <returns>true if data is retrieved successfully; otherwise, false.</returns>
        Public Function ChkDuplicateByMS_CABINET_ID(cMS_CABINET_ID As Long, cID As Long, trans As SQLTransaction) As Boolean
            Dim cmdPara(2)  As SQLParameter
            cmdPara(0) = DB.SetText("@_MS_CABINET_ID", cMS_CABINET_ID) 
            cmdPara(1) = DB.SetBigInt("@_ID", cID) 
            Return doChkData("MS_CABINET_ID = @_MS_CABINET_ID And ID <> @_ID", trans, cmdPara)
        End Function
        ...
        Private Function SetParameterData() As SqlParameter()
            ...
            cmbParam(13) = New SqlParameter("@_MS_CABINET_ID", SqlDbType.BigInt)
            cmbParam(13).Value = _MS_CABINET_ID

            cmbParam(14) = New SqlParameter("@_SYNC_TO_KIOSK", SqlDbType.Char)
            cmbParam(14).Value = _SYNC_TO_KIOSK
        ...
        '/// <param name=trans>The System.Data.SQLClient.SQLTransaction used by this System.Data.SQLClient.SQLCommand.</param>
        '/// <returns>true if data is retrieved successfully; otherwise, false.</returns>
        Private Function doGetData(whText As String, trans As SQLTransaction, cmdPara() As SQLParameter) As MsCabinetKioskLinqDB
            ClearData()
            _haveData  = False
            If whText.Trim() <> "" Then
                Dim tmpWhere As String = " WHERE " & whText
        ...
        Private ReadOnly Property SqlInsert() As String 
            Get
                Dim Sql As String=""
                Sql += "INSERT INTO " & tableName  & " (CREATED_BY, CREATED_DATE, MS_KIOSK_ID, MS_CABINET_MODEL_ID, CABINET_NO, ORDER_LAYOUT, SERVICE_RATE_HOUR, SERVICE_RATE_LIMIT_DAY, DEPOSIT_AMT, ACTIVE_STATUS, MS_CABINET_ID, SYNC_TO_KIOSK, SYNC_TO_SERVER)"
                Sql += " OUTPUT INSERTED.ID, INSERTED.CREATED_BY, INSERTED.CREATED_DATE, INSERTED.UPDATED_BY, INSERTED.UPDATED_DATE, INSERTED.MS_KIOSK_ID, INSERTED.MS_CABINET_MODEL_ID, INSERTED.CABINET_NO, INSERTED.ORDER_LAYOUT, INSERTED.SERVICE_RATE_HOUR, INSERTED.SERVICE_RATE_LIMIT_DAY, INSERTED.DEPOSIT_AMT, INSERTED.ACTIVE_STATUS, INSERTED.MS_CABINET_ID, INSERTED.SYNC_TO_KIOSK, INSERTED.SYNC_TO_SERVER"
                Sql += " VALUES("
                sql += "@_CREATED_BY" & ", "
                sql += "@_CREATED_DATE" & ", "
                sql += "@_MS_KIOSK_ID" & ", "
        ...
        Private ReadOnly Property SqlSelect() As String
            Get
                Dim Sql As String = "SELECT ID, CREATED_BY, CREATED_DATE, UPDATED_BY, UPDATED_DATE, MS_KIOSK_ID, MS_CABINET_MODEL_ID, CABINET_NO, ORDER_LAYOUT, SERVICE_RATE_HOUR, SERVICE_RATE_LIMIT_DAY, DEPOSIT_AMT, ACTIVE_STATUS, MS_CABINET_ID, SYNC_TO_KIOSK, SYNC_TO_SERVER FROM " & tableName
                Return Sql
            End Get
        End Property
