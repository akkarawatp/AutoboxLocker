...
#End If
    ...
    Public Function ConnectToDevice(ByVal deviceIndex As Integer, ByVal frameRate As Integer, ByVal windowSize As Size, ByVal frameSize As FrameSizes, ByVal hParent As IntPtr) As Boolean
        ...
        If (deviceIndex + 1 > capDevices.Length) Then Throw New Exception("ไม่พบอุปกรณ์จับภาพในคอมพิวเตอร์เครื่องนี้")
        ...
        Catch
            ...
            If unsupportedVideo Then
                MessageBox.Show("ขนาดความละเอียดของภาพไม่รองรับกับอุปกรณ์")
            Else
                MessageBox.Show("ผิดพลาดไม่สามารถเชื่อมต่อกล้องได้")
            End If
            Return False
        End Try
