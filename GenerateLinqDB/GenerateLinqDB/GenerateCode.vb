...
Public Class GenerateCode
    ...
    Private Function GetVBData(ByVal type As String, ByVal dr As DataRow, ByVal className As String, LinqDBFolder As String) As VBf.GenerateData
        Dim data As New VBf.GenerateData
        data.DataSource = txtDataSource.Text.Trim()
        data.DataBaseName = txtDatabaseName.Text.Trim()
        data.UserID = txtUserID.Text.Trim()
        ...
        data.DatabaseType = "SQL"
        If rdiKioskDB.Checked = True Then
            data.DatabaseClass = "KioskDB"
        Else
            data.DatabaseClass = "ServerDB"
        End If
    ...
    Private Sub GenerateTable(ByVal dr As DataRow)
        Dim LinqDBFolder As String = ""
        If rdiKioskDB.Checked = True Then
            LinqDBFolder = "KioskLinqDB"
        Else
            LinqDBFolder = "ServerLinqDB"
        End If

        Dim TabName As String = LinqDBFolder & "_" & dr("TABLE_NAME")
        ...
        tabLinq.TabPages.Add(TabName, TabName)
        ...
        _sqlVBFlow = New VBf.SqlGenerateFlow
        txtLinq.Text = _sqlVBFlow.GenerateCodeDAL(GetVBData(LinqDBFolder, dr, className, LinqDBFolder))
        tabLinq.TabPages(TabName).Controls.Add(txtLinq)
